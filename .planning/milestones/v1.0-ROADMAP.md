...
**Key Decisions:**
- Use subprocess-based container lifecycle (pytest-docker deferred)
- Module-scoped fixtures for expensive gRPC connections
- FastMCP migration for MCP SDK 1.25.0 compatibility
- Tenacity library for retry logic with exponential backoff
- Skip guards for unavailable test targets (Windows, SSH, WinRM)
- Placeholder baselines populated by live test execution

**Issues Resolved:**
- MCP tool invocation broken (fixed via FastMCP migration)
- Artifact collection returning empty results (fixed source() VQL syntax)
- Stack trace exposure in error responses (fixed in all tool modules)

**Issues Deferred to v2:**
- pytest-docker plugin integration
- Windows artifact testing (requires Windows target)
- SSH/WinRM agent deployment testing (requires targets)
- Cloud deployment testing (AWS/Azure)

**Technical Debt Incurred:**
- Phase 2 VERIFICATION.md is stale (pre-gap-closure)
- Placeholder baseline files awaiting live population

---

*Roadmap created: 2026-01-25*
*Milestone shipped: 2026-01-26*
